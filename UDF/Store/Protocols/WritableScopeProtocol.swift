...
/// The ``WritableScopeProtocol``  provides methods for ``Store`` scoping.
/// This protocol works with mutable state properties.
/// For immutable properties see ``ScopeProtocol``
public protocol WritableScopeProtocol {
...
public extension WritableScopeProtocol {

    /// Scopes the store to a local state.
    ///
    /// - Parameter keypath: A keypath for a `LocalState`.
    /// - Returns: A `Store` with scoped `State`.
    func scope<LocalState>(_ keyPath: WritableKeyPath<State, LocalState>) -> AnyWritableProxyStore<LocalState> {
    ...
    /// - Returns: A `Store` with scoped `State`.
    func scope<LocalState: Equatable>(_ keyPath: WritableKeyPath<State, LocalState>) -> AnyWritableProxyStore<LocalState> {
